          ...
          {b.photos.map((photo, i) => (
            ...
            >
              ...
              <img
                ...
                alt={photo.title}
                loading="lazy"
                className={`w-full object-cover ${PIN_ASPECTS[i % PIN_ASPECTS.length]}`}
              />
              <button
                type="button"
                onClick={() => unsave.mutate({ boardId: id, photoId: photo.id })}
                className="absolute right-2.5 top-2.5 rounded-full bg-foreground/90 px-3 py-1.5 text-xs font-semibold text-background opacity-0 shadow-md transition-opacity hover:bg-foreground group-hover:opacity-100"
              >
                {t('removePin')}
              </button>
