...
// Dressing v3 (feedback PO 2026-07-13, docs/12 S3):
// - iluminarea LED devine OPTIUNE in modulele interioare (langa accesorii),
//   intrebarea separata de lighting dispare — un singur loc pentru interior.
// v2 ramane FROZEN in dressing.v2.ts (cererile publicate/drafturile vechi
// se valideaza contra ei).
...
// LED_LIGHTING sta langa ACCESSORIES: tot "interiorul" se alege intr-un loc
const MODULE_DEFS = [
  ...
  { value: 'LED_LIGHTING', icon: 'lightbulb' },
];

export const dressingFlowV3: RoomFlow = {
  roomType: 'DRESSING',
  version: 3,
  steps: [
    {
  ...
  deriveRoom: (answers) => {
    ...
    const furnishings = modules.filter((m) => m !== 'LED_LIGHTING');
    if (furnishings.length > 0) details.push(`Interior: ${furnishings.join(', ')}`);
    ...
    if (modules.includes('LED_LIGHTING')) details.push('iluminare LED');
    return {
