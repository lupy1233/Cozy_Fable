...
WORKDIR /app
# workspace-ul complet (backend dist + node_modules + prisma)
COPY --from=build /app ./
# frontend standalone izolat sub /app/web (are node_modules propriu, minimal)
COPY --from=build /app/apps/frontend/.next/standalone /app/web
COPY --from=build /app/apps/frontend/.next/static /app/web/apps/frontend/.next/static
COPY --from=build /app/apps/frontend/public /app/web/apps/frontend/public
